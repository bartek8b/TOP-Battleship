...
describe('Gameboard', () => {
  ...
  test('Properly tracks accurate shot and sunk cells', () => {
    const gameboard = new Gameboard();
    ...
    expect(gameboard.grid[0][1]).toBe('sunk');
    expect(gameboard.grid[0][2]).toBe('sunk');
    expect(gameboard.grid[0][5]).toBe('miss');

    // Sunk cells recorded
    expect(gameboard.sunkCells.has('0,1')).toBe(true);
    expect(gameboard.sunkCells.has('0,2')).toBe(true);
